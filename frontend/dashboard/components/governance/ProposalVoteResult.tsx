...
export interface ProposalVoteResultProps {
  votesFor: number;
  votesAgainst: number;
  votesAbstain?: number;
...
export const ProposalVoteResult: React.FC<ProposalVoteResultProps> = ({
  votesFor,
  votesAgainst,
  votesAbstain,
  totalEligible,
  ...
  return (
    <section className="bg-card border rounded-2xl shadow p-6 w-full max-w-xl mx-auto mb-6">
      ...
      <div className="flex flex-col gap-2 mb-3">
        <VoteBar label="For" color="bg-green-500" value={votesFor} percent={forPct} />
        <VoteBar label="Against" color="bg-red-500" value={votesAgainst} percent={againstPct} />
        {votesAbstain !== undefined && (
          <VoteBar label="Abstain" color="bg-gray-400" value={votesAbstain} percent={abstainPct} />
        )}
      </div>
      <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-muted-foreground mt-3">
        <span className="flex items-center gap-1">
          <Users className="w-4 h-4" /> Turnout: <b>{turnout !== undefined ? turnout + "%" : totalVotes}</b>
        </span>
        {totalEligible !== undefined && <span>Eligible: <b>{totalEligible}</b></span>}
        {quorum !== undefined && <span>Quorum: <b>{quorum}</b></span>}
        {threshold !== undefined && <span>Threshold: <b>{threshold}</b></span>}
        {status && <span>Status: <b className="capitalize">{status}</b></span>}
      </div>
